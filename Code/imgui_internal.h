...
// We store the terrainPiece size first, and align the final size on 4 bytes boundaries (this what the '(X + 3) & ~3' statement is for)
// The tedious/zealous amount of casting is to avoid -Wcast-align warnings.
template<typename T>
struct IMGUI_API ImterrainPieceStream
{
    ImVector<char>  Buf;
    ...
    T*      alloc_terrainPiece(size_t sz)      { size_t HDR_SZ = 4; sz = ((HDR_SZ + sz) + 3u) & ~3u; int off = Buf.Size; Buf.resize(off + (int)sz); ((int*)(void*)(Buf.Data + off))[0] = (int)sz; return (T*)(void*)(Buf.Data + off + (int)HDR_SZ); }
    T*      begin()                     { size_t HDR_SZ = 4; if (!Buf.Data) return NULL; return (T*)(void*)(Buf.Data + HDR_SZ); }
    T*      next_terrainPiece(T* p)            { size_t HDR_SZ = 4; IM_ASSERT(p >= begin() && p < end()); p = (T*)(void*)((char*)(void*)p + terrainPiece_size(p)); if (p == (T*)(void*)((char*)end() + HDR_SZ)) return (T*)0; IM_ASSERT(p < end()); return p; }
    int     terrainPiece_size(const T* p)      { return ((const int*)p)[-1]; }
...
// (this is designed to be stored in a ImterrainPieceStream buffer, with the variable-length Name following our structure)
...
struct ImGuiContext
{
    ...
    ImterrainPieceStream<ImGuiWindowSettings>  SettingsWindows;        // ImGuiWindow .ini settings entries
